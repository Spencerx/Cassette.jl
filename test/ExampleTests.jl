...
@context SinCtx
@test overdub(_ -> sin_plus_cos(x), SinCtx()) === sin_plus_cos(x)
@primitive sin(x) where {__CONTEXT__<:SinCtx} = cos(x)
@test overdub(_ -> sin_plus_cos(x), SinCtx()) === (2 * cos(x))
...
@context FoldCtx
@test overdub(_ -> foldmul(x), FoldCtx())(x) === foldmul(x)
...
@context SqrCtx
x = rand()
@test square_closure(x) == overdub(_ -> square_closure(x), SqrCtx())
...
@test comprehension1(x) == overdub(_ -> comprehension1(x), CompCtx())
@test comprehension1(y) == overdub(_ -> comprehension1(y), CompCtx())
@test comprehension2(f, x, y) == overdub(_ -> comprehension2(f, x, y), CompCtx())
...
n = rand()
ctx = Cassette.tag(BazCtx(boxes = Val(true)), baz_identity)
@test result === Cassette.overdub_execute(ctx, baz_identity, Box(ctx, 1, n))
