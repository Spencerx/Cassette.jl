...
prehook(::AbstractContext, ::Val{w}, ::Vararg{Any}) where {w} = nothing
posthook(::AbstractContext, ::Val{w}, ::Vararg{Any}) where {w} = nothing
is_user_primitive(ctx::AbstractContext, ::Val{w}, ::Vararg{Any}) where {w} = false
is_core_primitive(ctx::AbstractContext, ::Val{w}, args...) where {w} = _is_core_primitive(ctx, args...)
execution(ctx::AbstractContext, ::Val{w}, f, args...) where {w} = unboxcall(ctx, f, args...)
...
overdub(f, ctx::AbstractContext{w,b,P,Nothing}) where {w,b,P} = overdub(f, tag(ctx, f))
overdub(f, ctx::AbstractContext{w,b,P,<:AbstractTag}) where {w,b,P} = overdub_recurse(ctx, f)

###################
# overdub_execute #
###################

function overdub_execute(ctx::AbstractContext, args...)
...
function overdub_recurse_pass!(reflection::Reflection,
    ...
    # expression and replace it with a corresponding call to `Cassette.overdub_recurse`.
    for i in length(overdubbed_code):length(code_info.code)
        stmnt = code_info.code[i]
        replace_match!(is_call, stmnt) do call
            call.args = Any[GlobalRef(Cassette, :overdub_recurse), overdub_ctx_slot, call.args...]
            return call
        end
